import 'package:layout/entity/plu_model.dart';
...
class CoCheck{
  ...
  factory CoCheck.fromJson(Map<String, dynamic> json) => CoCheck(
    ...
    rows: (json['Rows'] != null)
        ? List<RowEntity>.from(json['Rows'].map((x) => RowEntity.fromJson(x)))
        : null,
    plus: (json['PLUs'] != null)
        ? List<PLUsEntity>.from(json['PLUs'].map((x) => PLUsEntity.fromJson(x)))
        : null,
    data: json['Data'],
  );
}